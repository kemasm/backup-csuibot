...
class TestMeme:
    ...
    def test_generate_meme(self):
        url_regex = 'http://i.imgflip.com(.*)'
        result = utils.generate_meme('foo', 'bar')
        assert bool(re.match(url_regex, result))

    def test_failed_meme(self, mocker):
        error_message = 'Error happened, cannot generate meme.'
        class FakeMeme():
            def build_meme(self, *args, **kwargs):
                return 'Error happened, cannot generate meme.'
                
        mocker.patch('csuibot.utils.m.MemeBuilder', return_value=FakeMeme())
        res = utils.generate_meme('foo', 'bar')
